...
boru-driven command-line client **and** interactive REPL for the LasVegasCity
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/las-vegas-city-cli)
make build

# 2. See usage (words, entities, env vars)
./las-vegas-city-cli --help

# 3. Provide credentials once, via the environment
export LAS_VEGAS_CITY_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./las-vegas-city-cli load 1 city_info            # {id:1} shorthand
./las-vegas-city-cli load '{id:1}' city_info       # explicit match map

# 5. Override the API base URL for a single call
LAS_VEGAS_CITY_BASE=https://api.example.com ./las-vegas-city-cli load 1 city_info

# 6. No arguments -> interactive REPL
./las-vegas-city-cli
las-vegas-city> load 1 city_info
las-vegas-city> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/las-vegas-city-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/las-vegas-city-cli load 1 city_info
   ```
...
```sh
./las-vegas-city-cli load 1 city_info          # scalar shorthand for {id:1}
./las-vegas-city-cli load '{id:1}' city_info     # explicit match map
```
...
./las-vegas-city-cli load 1 city_info
```
...
Run with no arguments to open a REPL (prompt `las-vegas-city>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `city_info`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/las-vegas-city-cli`. |
...
(`./las-vegas-city-cli <expr>`) and interactively (the REPL), and expressions compose the
