// Command LasVegasCity-cli is a CLI + REPL for the LasVegasCity SDK,
...
const prompt = "las-vegas-city"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "city_info council department economic_development event job meeting new park permit public_safety"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      LAS_VEGAS_CITY_APIKEY (api key), LAS_VEGAS_CITY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: LAS_VEGAS_CITY_APIKEY carries the API key and
	// LAS_VEGAS_CITY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LAS_VEGAS_CITY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LAS_VEGAS_CITY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewLasVegasCitySDK(opts)
