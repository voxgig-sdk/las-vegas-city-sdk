...
func TestMeetingEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LASVEGASCITY_TEST_MEETING_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func meetingBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LASVEGASCITY_TEST_MEETING_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LASVEGASCITY_TEST_MEETING_ENTID"])
