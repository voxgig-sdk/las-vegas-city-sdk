...
class PublicSafetyEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = LasVegasCitySDK::test(null, null);
        $ent = $testsdk->PublicSafety(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = public_safety_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "public_safety." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LASVEGASCITY_TEST_PUBLIC_SAFETY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $public_safety_ref01_data = null;
        if (count($public_safety_ref01_data_raw) > 0) {
...
function public_safety_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/public_safety/PublicSafetyTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["public_safety01", "public_safety02", "public_safety03"] as $k) {
    ...
    $entid_env_raw = getenv("LASVEGASCITY_TEST_PUBLIC_SAFETY_ENTID");
