...
module LasVegasCityUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = LasVegasCityResponse.new({})
    ...
    if fetch_err
      ...
      response = LasVegasCityResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LasVegasCityResponse.new(fetched)
    else
