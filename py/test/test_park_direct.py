...
class TestParkDirect:

    def test_should_direct_list_park(self):
        setup = _park_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-park", "live" if setup["live"] else "unit")
        if _skip:
...
def _park_direct_setup(mockres):
