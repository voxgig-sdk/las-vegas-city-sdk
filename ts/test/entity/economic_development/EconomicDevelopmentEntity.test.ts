...
describe('EconomicDevelopmentEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'economic_development.' + op, live)) return
    }
    ...
    const economic_development_ref01_list = await economic_development_ref01_ent.list(economic_development_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LAS_VEGAS_CITY_TEST_ECONOMIC_DEVELOPMENT_ENTID']
