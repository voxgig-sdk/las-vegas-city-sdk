...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.LASVEGASCITY_TEST_LIVE ||
    'TRUE' === process.env.LASVEGASCITY_TEST_OVERRIDE
  ) {
  ...
  m.LASVEGASCITY_TEST_EXPLAIN = process.env.LASVEGASCITY_TEST_EXPLAIN || m.LASVEGASCITY_TEST_EXPLAIN
