...
func TestEventEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LAS_VEGAS_CITY_TEST_EVENT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func eventBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LAS_VEGAS_CITY_TEST_EVENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LAS_VEGAS_CITY_TEST_EVENT_ENTID"])
