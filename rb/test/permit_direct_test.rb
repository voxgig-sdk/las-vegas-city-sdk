...
class PermitDirectTest < Minitest::Test
  def test_direct_list_permit
    setup = permit_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-permit", setup[:live] ? "live" : "unit")
    if _should_skip
...
def permit_direct_setup(mockres)
