...
class EconomicDevelopmentEntity
  ...
  def make
    opts = @_entopts.dup
    EconomicDevelopmentEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = LasVegasCityHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [EconomicDevelopment, Hash] the current EconomicDevelopment data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = LasVegasCityHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of EconomicDevelopment fields)
  def match_get
  ...
  #
  # @param reqmatch [EconomicDevelopmentListMatch, Hash, nil] match filter (any subset of
  #   EconomicDevelopment fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<EconomicDevelopment>, Array] the matching EconomicDevelopment items; raises LasVegasCityError on failure
