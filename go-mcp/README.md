...
[MCP](https://modelcontextprotocol.io) server exposing the LasVegasCity SDK as
two agent tools — `las-vegas-city_list` and `las-vegas-city_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/las-vegas-city-mcp)
make build

# 2. Provide credentials via the environment
export LAS_VEGAS_CITY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user las-vegas-city \
  -- /absolute/path/to/las-vegas-city-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./las-vegas-city-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// las-vegas-city_list: first page of records
{ "entity": "council" }
{ "entity": "council", "query": { } }

// las-vegas-city_load: one record by id
{ "entity": "city_info", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/las-vegas-city-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user las-vegas-city \
     -- "$PWD"/dist/*/las-vegas-city-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `las-vegas-city_list` and `las-vegas-city_load` tools now appear
   in new sessions. Ask the agent to *"list council using las-vegas-city"*
   and it calls `las-vegas-city_list` with `{"entity":"council"}`.
...
```sh
export LAS_VEGAS_CITY_APIKEY=sk_live_xxx            # API key
export LAS_VEGAS_CITY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `las-vegas-city_list` tool
...
### Call the `las-vegas-city_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `las-vegas-city_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `las-vegas-city_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LAS_VEGAS_CITY_APIKEY` | API key sent with every request. |
| `LAS_VEGAS_CITY_BASE` | Optional override of the API base URL. |

### Entities

The 11 entities valid as the `entity` argument:

city_info | council | department | economic_development | event | job | meeting | new | park | permit | public_safety

### Smoke test via HTTP (raw JSON-RPC)

```sh
./las-vegas-city-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"las-vegas-city_load","arguments":{"entity":"city_info","query":{"id":1}}}}'
```
