...
class TestPermitDirect:

    def test_should_direct_list_permit(self):
        setup = _permit_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-permit", "live" if setup["live"] else "unit")
        if _skip:
...
def _permit_direct_setup(mockres):
