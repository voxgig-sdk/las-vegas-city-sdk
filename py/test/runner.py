...
class LasVegasCityTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            LasVegasCityTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = LasVegasCityTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = LasVegasCityTestRunner.getenv("LAS_VEGAS_CITY_TEST_LIVE")
        override = LasVegasCityTestRunner.getenv("LAS_VEGAS_CITY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = LasVegasCityTestRunner.getenv(key)
        ...
        explain = LasVegasCityTestRunner.getenv("LAS_VEGAS_CITY_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["LAS_VEGAS_CITY_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if LasVegasCityTestRunner._test_control is not None:
            return LasVegasCityTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                LasVegasCityTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            LasVegasCityTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = LasVegasCityTestRunner.load_test_control()
...
def load_env_local():
    LasVegasCityTestRunner.load_env_local()


def env_override(m):
    return LasVegasCityTestRunner.env_override(m)


def entity_data(v):
    return LasVegasCityTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return LasVegasCityTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return LasVegasCityTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return LasVegasCityTestRunner.load_test_control()


def live_client_options():
    return LasVegasCityTestRunner.live_client_options()


def live_delay_ms():
    return LasVegasCityTestRunner.live_delay_ms()
