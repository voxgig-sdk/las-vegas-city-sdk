...
// LasVegasCity SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class LasVegasCityMakePoint
{
    public static function call(LasVegasCityContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof LasVegasCityError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = LasVegasCityHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = LasVegasCityHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
