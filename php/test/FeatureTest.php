...
class FtCtrl extends LasVegasCityControl
{
...
class FtClient
{
    ...
    private LasVegasCityUtility $utility;

    public function __construct(LasVegasCityUtility $utility, array $options)
    {
    ...
    public function get_utility(): LasVegasCityUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LasVegasCityContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public LasVegasCityContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LasVegasCityContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LasVegasCityConfig::make_feature($name);
    ...
    private static function buildUrl(LasVegasCitySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LasVegasCityError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LasVegasCityResponse($fetched);
        }
    ...
    private function fail(LasVegasCityContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LasVegasCityContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
