...
// LasVegasCity SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class LasVegasCityTestFeature extends LasVegasCityBaseFeature
{
    ...
    public function init(LasVegasCityContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (LasVegasCityContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = LasVegasCityParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (LasVegasCityContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(LasVegasCityContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = LasVegasCityParam::call($ctx, $k);
