...
class TestCouncilDirect:

    def test_should_direct_list_council(self):
        setup = _council_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-council", "live" if setup["live"] else "unit")
        if _skip:
...
def _council_direct_setup(mockres):
