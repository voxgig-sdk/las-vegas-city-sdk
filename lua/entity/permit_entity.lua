...
local PermitEntity = {}
PermitEntity.__index = PermitEntity


function PermitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PermitEntity)
  self._name = "permit"
  self._client = client
...
function PermitEntity:get_name()
  return self._name
end


function PermitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PermitEntity.new(self._client, opts)
end
...
function PermitEntity:mark_deleted()
  self._deleted = true
end


function PermitEntity:deleted()
  return true == self._deleted
end


function PermitEntity:data_set(args)
  if args ~= nil then
...
function PermitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PermitEntity:match_set(args)
  if args ~= nil then
...
function PermitEntity:match_get()
...
function PermitEntity:stream(action, args, callopts)
...
---@param reqmatch PermitListMatch
---@param ctrl? table
---@return Permit[]
---@return string? err
function PermitEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PermitEntity:_run_op(ctx, post_done)
  local utility = self._utility
