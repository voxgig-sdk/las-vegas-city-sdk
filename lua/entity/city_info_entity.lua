...
local CityInfoEntity = {}
CityInfoEntity.__index = CityInfoEntity


function CityInfoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CityInfoEntity)
...
function CityInfoEntity:get_name()
  return self._name
end


function CityInfoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CityInfoEntity.new(self._client, opts)
end
...
function CityInfoEntity:mark_deleted()
  self._deleted = true
end


function CityInfoEntity:deleted()
  return true == self._deleted
end


function CityInfoEntity:data_set(args)
  if args ~= nil then
...
function CityInfoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CityInfoEntity:match_set(args)
  if args ~= nil then
...
function CityInfoEntity:match_get()
...
function CityInfoEntity:stream(action, args, callopts)
...
---@param reqmatch CityInfoLoadMatch
---@param ctrl? table
---@return CityInfo
---@return string? err
function CityInfoEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CityInfoEntity:_run_op(ctx, post_done)
  local utility = self._utility
