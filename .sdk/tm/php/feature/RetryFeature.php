...
// LasVegasCity SDK retry feature
...
class LasVegasCityRetryFeature extends LasVegasCityBaseFeature
{
    ...
    public function init(LasVegasCityContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (LasVegasCityContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(LasVegasCityContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(LasVegasCityContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
