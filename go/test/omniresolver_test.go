...
import (
	...
	omni "github.com/voxgig-sdk/las-vegas-city-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.LasVegasCityError); is {
				out := map[string]any{
					"name":    "LasVegasCityError",
...
func omniCtx(arg any, client *sdk.LasVegasCitySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.LasVegasCitySDK); is {
			client = live
