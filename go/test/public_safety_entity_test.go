...
func TestPublicSafetyEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LAS_VEGAS_CITY_TEST_PUBLIC_SAFETY_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		publicSafetyRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.public_safety")))
...
func public_safetyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LAS_VEGAS_CITY_TEST_PUBLIC_SAFETY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LAS_VEGAS_CITY_TEST_PUBLIC_SAFETY_ENTID"])
