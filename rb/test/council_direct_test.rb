...
class CouncilDirectTest < Minitest::Test
  def test_direct_list_council
    setup = council_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-council", setup[:live] ? "live" : "unit")
    if _should_skip
...
def council_direct_setup(mockres)
