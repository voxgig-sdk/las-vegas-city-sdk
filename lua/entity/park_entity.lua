...
local ParkEntity = {}
ParkEntity.__index = ParkEntity


function ParkEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ParkEntity)
  self._name = "park"
  self._client = client
...
function ParkEntity:get_name()
  return self._name
end


function ParkEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ParkEntity.new(self._client, opts)
end
...
function ParkEntity:mark_deleted()
  self._deleted = true
end


function ParkEntity:deleted()
  return true == self._deleted
end


function ParkEntity:data_set(args)
  if args ~= nil then
...
function ParkEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ParkEntity:match_set(args)
  if args ~= nil then
...
function ParkEntity:stream(action, args, callopts)
...
function ParkEntity:_run_op(ctx, post_done)
  local utility = self._utility
