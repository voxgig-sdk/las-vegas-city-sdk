...
local PublicSafetyEntity = {}
PublicSafetyEntity.__index = PublicSafetyEntity


function PublicSafetyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PublicSafetyEntity)
  self._name = "public_safety"
  self._client = client
...
function PublicSafetyEntity:get_name()
  return self._name
end


function PublicSafetyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PublicSafetyEntity.new(self._client, opts)
end
...
function PublicSafetyEntity:mark_deleted()
  self._deleted = true
end


function PublicSafetyEntity:deleted()
  return true == self._deleted
end


function PublicSafetyEntity:data_set(args)
  if args ~= nil then
...
function PublicSafetyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PublicSafetyEntity:match_set(args)
  if args ~= nil then
...
function PublicSafetyEntity:match_get()
...
function PublicSafetyEntity:stream(action, args, callopts)
...
function PublicSafetyEntity:_run_op(ctx, post_done)
  local utility = self._utility
