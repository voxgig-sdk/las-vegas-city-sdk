...
class ParkDirectTest < Minitest::Test
  def test_direct_list_park
    setup = park_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-park", setup[:live] ? "live" : "unit")
    if _should_skip
