...
local CouncilEntity = {}
CouncilEntity.__index = CouncilEntity


function CouncilEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CouncilEntity)
  self._name = "council"
  self._client = client
...
function CouncilEntity:get_name()
  return self._name
end


function CouncilEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CouncilEntity.new(self._client, opts)
end
...
function CouncilEntity:mark_deleted()
  self._deleted = true
end


function CouncilEntity:deleted()
  return true == self._deleted
end


function CouncilEntity:data_set(args)
  if args ~= nil then
...
function CouncilEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CouncilEntity:match_set(args)
  if args ~= nil then
...
function CouncilEntity:match_get()
...
function CouncilEntity:stream(action, args, callopts)
...
function CouncilEntity:_run_op(ctx, post_done)
  local utility = self._utility
