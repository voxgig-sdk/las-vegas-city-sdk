...
The Lua SDK for the LasVegasCity API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:CityInfo(nil):load({ id = "example_id" }, nil)
...
local result, err = client:LasVegasCity(nil):load(
  { id = "test01" }, nil
)
