...
The Python SDK for the LasVegasCity API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.CityInfo(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.LasVegasCity(None).load(
    {"id": "test01"}, None
)
