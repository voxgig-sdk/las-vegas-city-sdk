...
class PublicSafetyEntity
  ...
  def make
    opts = @_entopts.dup
    PublicSafetyEntity.new(@_client, opts)
  end
  ...
  # @return [PublicSafety, Hash] the current PublicSafety data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of PublicSafety fields)
  def match_get
  ...
  #
  # @param reqmatch [PublicSafetyLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.PublicSafety.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [PublicSafety, Hash] the loaded PublicSafety; raises LasVegasCityError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = LasVegasCityHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
