...
// LasVegasCity SDK pipeline test
//
...
{
    private static function utility(): LasVegasCityUtility
    {
        return new LasVegasCityUtility();
    }
    ...
    private static function ctx(array $over = []): LasVegasCityContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new LasVegasCityContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new LasVegasCityOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof LasVegasCityError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new LasVegasCityOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = LasVegasCityMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new LasVegasCityOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = LasVegasCityMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new LasVegasCityOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = LasVegasCityMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new LasVegasCitySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = LasVegasCityMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new LasVegasCityResponse([]),
            'result' => new LasVegasCityResult([]),
        ]);
        [, $err] = LasVegasCityMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new LasVegasCitySpec([]),
            'response' => null,
            'result' => new LasVegasCityResult([]),
        ]);
        [, $err] = LasVegasCityMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new LasVegasCitySpec(['step' => 's']),
            'response' => new LasVegasCityResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new LasVegasCityResult([]),
        ]);
        [, $err] = LasVegasCityMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new LasVegasCitySpec(['step' => 's']),
            'response' => new LasVegasCityResponse(self::resp(200, ['v' => 1])),
            'result' => new LasVegasCityResult([]),
        ]);
        [, $err] = LasVegasCityMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new LasVegasCitySpec(['step' => 's']),
            'response' => new LasVegasCityResponse(self::resp(200, ['v' => 2])),
            'result' => new LasVegasCityResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        LasVegasCityMakeResponse::call($ctx);
    ...
    {
        $preset = new LasVegasCityResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new LasVegasCitySpec([]),
            'response' => new LasVegasCityResponse([]),
            'result' => new LasVegasCityResult([]),
        ...
        [$got, $err] = LasVegasCityMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new LasVegasCityResult([])]);
        [, $err] = LasVegasCityMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new LasVegasCitySpec([]), 'result' => null]);
        [, $err] = LasVegasCityMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new LasVegasCitySpec(['step' => 's']),
            'result' => new LasVegasCityResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new LasVegasCityOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = LasVegasCityMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new LasVegasCitySpec(['step' => 's']),
            'result' => new LasVegasCityResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new LasVegasCityOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = LasVegasCityMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new LasVegasCityResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new LasVegasCitySpec([]),
            'result' => new LasVegasCityResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = LasVegasCityMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = LasVegasCityMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new LasVegasCityError('boom', 'boom');
        $utility->fetcher = function (LasVegasCityContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new LasVegasCitySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = LasVegasCityMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (LasVegasCityContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = LasVegasCityMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(LasVegasCityResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        LasVegasCityMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (LasVegasCityContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new LasVegasCitySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = LasVegasCityMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new LasVegasCityResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new LasVegasCitySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = LasVegasCityMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = LasVegasCityMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = LasVegasCityMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new LasVegasCityResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, LasVegasCityDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new LasVegasCityResult(['ok' => false])]);
        $this->expectException(LasVegasCityError::class);
        LasVegasCityDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new LasVegasCityResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', LasVegasCityMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        LasVegasCityMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new LasVegasCityBaseFeature();
        $b = new LasVegasCityBaseFeature();
        LasVegasCityFeatureAdd::call($ctx, $a);
        LasVegasCityFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): LasVegasCityBaseFeature
    {
        $f = new LasVegasCityBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        LasVegasCityFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): LasVegasCityContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new LasVegasCitySpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = LasVegasCityPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        LasVegasCityPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new LasVegasCityResponse(['headers' => null]),
            'result' => new LasVegasCityResult([]),
        ]);
        LasVegasCityResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        LasVegasCityResultBody::call($ctx);
