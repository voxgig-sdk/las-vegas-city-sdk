...
func (sdk *LasVegasCitySDK) OptionsMap() map[string]any {
...
func (sdk *LasVegasCitySDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *LasVegasCitySDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *LasVegasCitySDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *LasVegasCitySDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *LasVegasCitySDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *LasVegasCitySDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("LasVegasCitySDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *LasVegasCitySDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *LasVegasCitySDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("LasVegasCitySDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *LasVegasCitySDK {
	...
	sdk := NewLasVegasCitySDK(sdkopts)
	sdk.Mode = "test"
