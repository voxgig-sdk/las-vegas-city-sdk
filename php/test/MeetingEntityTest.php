...
class MeetingEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LASVEGASCITY_TEST_MEETING_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function meeting_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("LASVEGASCITY_TEST_MEETING_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "LASVEGASCITY_TEST_MEETING_ENTID" => $idmap,
        "LASVEGASCITY_TEST_LIVE" => "FALSE",
        "LASVEGASCITY_TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["LASVEGASCITY_TEST_MEETING_ENTID"]);
